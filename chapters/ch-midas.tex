...
Damit die relevanten Informationen für den Algorithmus mit einem konstanten Speicher verarbeitet werden, wird Count-Min-Sketch genutzt, dass einer Streaming-Datenstruktur mithilfe der Nutzung von Hash-Funktionen entspricht. Count-Min-Sketch zählt somit die Frequenz einer Aktivität in Datenströmen. Diese Datenstruktur hat ebenfalls den Vorteil, dass man zu Beginn keine Kenntnis über die Anzahl an Quell- und Zielpaaren haben muss. \citep{CMS04}
...
\begin{description}
	\item[$ \hat{s}_{uv}$]\hfill\\ Anzahl an Kanten zwischen Knoten u und Knoten v. Dieses Feature wird auch vom MIDAS Algorithmus verwendet.
	\item[$ \hat{s}_{u}$]\hfill\\ Gesamtanzahl an Nachbarknoten eines Knoten u.
	\item[$ \hat{a}_{u}$]\hfill\\ Aktuelle Anzahl an Nachbarknoten eines Knoten u.
\end{description}
