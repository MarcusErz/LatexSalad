...
Betrachtet man die Differenz aus dem Durchschnitt der Signaturvektoren und dem der Ausreißergraphen in einer \textit{Headmap}, dann kann man erkennen, dass die Ausreißer vorwiegend den besonders großen Ego-Netzwerken und eine hohe Anzahl an E-Mails verursacht werden. Vergleicht man die Zeitleiste (\citep{EnronTimeline}) mit den Ausreißerdaten, erkennt man einen starken Anstieg des Email-Verkehrs im Oktober 2001. Hier wurde von Enron ein Report veröffentlicht über einen Quartals-Verlust von 618 Millionen US-Dollar und und (über) eine Reduzierung des Eigenkapitals um 1.2 Milliarden. Im Dezember 2001 ist ein verhältnismäßig geringer Email-Verkehr festzustellen. Betrachtet man hier die Zeitleiste, so wurden in dieser Zeitspanne 4000 Mitarbeiter entlassen. \workTodo{Quelle die Webseite}
...
So hat bspw. das Feature $|E^{\circ}_{ego(i)}|$ in einem vollständigen Graphen,keine Aussagekraft, da jeder Knoten die gleiche Anzahl an Kanten in seinem Ego-Netzwerk aufweist. Subtrahiert man also vom durchschnittlichen Signaturvektor aller Graphen die einzelnen Signaturvektoren, so erkennt man den Wert 0 in allen Headmaps.

Somit müssen hierbei für vollständige Graphen andere Features extrahiert werden. Außerdem ist die Laufzeit in großen Datensätzen, wie z.B. dem Darpa-Datensatz, mit 3 Stunden Berechnungszeit nicht performant.
...
\workTodo{Wie kann das mit den 10 Prozent richtig dargestellt werden.}
%\begin{itemize}
...
%	\item $ \sqrt[n]{\prod\limits_{i = 1}^{n} x_i} $ \hfill\\ Geometrisches Mittel der Kantengewichte eines Knoten.
%%	\item $
%%	x(p) =
%%	\begin{cases}
...
%	\item $ sort(x) \to \sqrt[0.1*n]{\prod\limits_{i = 1}^{0.1*n} x_i} $ \hfill\\ Geometrisches Mittel der Kanten mit den 10\% höchsten Kantengewichten.
%	\item $ sort(x) \to \sqrt[0.2*n]{\prod\limits_{i = 1}^{0.2*n} x_i} $ \hfill\\ Geometrisches Mittel der Kanten mit den 20\% höchsten Kantengewichten. 
%\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{p{0.33\linewidth}|p{0.6\linewidth}}
		...
		\midrule
		$ sort(x) \to \sqrt[0.1*n]{\prod\limits_{i = 1}^{0.1*n} x_i} $ & Geometrisches Mittel der Kanten mit den 10\% höchsten Kantengewichten. \\
		\midrule
		$ sort(x) \to \sqrt[0.2*n]{\prod\limits_{i = 1}^{0.2*n} x_i} $ & Geometrisches Mittel der Kanten mit den 20\% höchsten Kantengewichten. \\
		\bottomrule
	\end{tabular}
